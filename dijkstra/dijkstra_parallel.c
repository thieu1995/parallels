...
void Print_dists_paths(int loc_dist[], int loc_pred[], int n, int loc_n, int my_rank, MPI_Comm comm);
...
//    Print_paths(loc_pred, n, loc_n, my_rank, comm);
    ...
    Print_dists_paths(loc_dist, loc_pred, n, loc_n, my_rank, comm);
...
void Print_dists_paths(int loc_dist[], int loc_pred[], int n, int loc_n, int my_rank, MPI_Comm comm) {
    ...
    if (my_rank == 0) {
        ...
        printf("The shortest path and distance from 0 to each vertex is:\n");
        printf("  v    Dist 0->v    Path 0->v\n");
        printf("----   ---------    ---------\n");

        for (v = 1; v < n; v++) {
            printf("%3d:    %4d        ", v, dist[v]);
            count = 0;
            w = v;
            while (w != 0) {
                path[count] = w;
